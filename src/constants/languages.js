...
export const getTier1Languages = () => LANGUAGES.filter((l) => l.tier === 1);
export const getTier2Languages = () => LANGUAGES.filter((l) => l.tier === 2);
export const getLanguageByCode = (code) => {
  const normalized = String(code || "").toLowerCase();
  const base = normalized.split("-")[0];
  return LANGUAGES.find((l) => l.code === normalized || l.code === base) || null;
};

// Backward compatibility for existing imports in ChatScreen.
const languages = LANGUAGES.map((l) => ({ code: l.code, label: l.name }));
