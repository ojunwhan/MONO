...
export default function HospitalPatientJoin() {
  ...
  const handleJoin = useCallback(async () => {
    ...
    try {
      // 1. Call POST /api/hospital/join to create a new room
      const res = await fetch("/api/hospital/join", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ department: department || "general" }),
      });
      ...
      const cleanName = getPatientLabel(selectedLang);

      // 2. Save guest session
      ...
      // 3. Navigate to chat room as guest
  ...
  }, [department, selectedLang, navigate]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={() => { setStep("language"); setShowLangGrid(true); }}
          style={{
            padding: "8px 24px",
            borderRadius: "8px",
